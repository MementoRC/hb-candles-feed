...
class MockedExchangeServer:
    ...
    @property
    def mocked_exchange_url(self):
        """Get the URL of the mock exchange server."""
        return self.plugin.adapter_class.get_rest_url()

    @property
    def url(self):
        """Get the URL of the mock exchange server."""
        if self.site is not None:
            return f"http://{self.site._host}:{self.site._port}" # Access the assigned port
        return f"http://{self.host}:{self.port}" # Fallback if the server hasn't started
    ...
    async def start(self) -> str:
        ...
        self._url = f"http://{self.host}:{self.site._port}"
        return self._url
    ...
    def _check_rate_limit(self, ip: str, api_type: str) -> bool:
        ...
        if api_type == "rest":
            ...
            if len(self.request_counts[api_type][ip]) >= burst:
                ...
                second_count = sum(
                    now - ts <= 1000 for ts in self.request_counts[api_type][ip]
                )
    ...
    async def handle_klines(self, request):
        ...
        and format the response. Supports Binance-specific parameters like
        timeZone for timezone adjustments.
        ...
        # Filter candles by time range, considering timezone if specified
        if timezone_offset_hours:
            # Adjust the candle timestamps by the timezone offset when filtering
            # This simulates returning candles aligned to the requested timezone
            timezone_offset_sec = int(timezone_offset_hours * 3600)
            filtered_candles = [
                candle
            ...
            # Adjust open/close times for formatting with timezone
            # We don't modify the original candle objects, just pass the offset to formatting
            params["timezone_adjustment_ms"] = int(timezone_offset_sec * 1000)
        else:
        ...
        # Format response based on exchange type with timezone adjustment if provided
        timezone_adjustment_ms = params.get("timezone_adjustment_ms", 0)
        response = self.plugin.format_rest_candles(
            limited_candles, 
            symbol, 
            interval, 
            timezone_adjustment_ms=timezone_adjustment_ms
        )
    ...
    async def _handle_ws_message(self, ws: web.WebSocketResponse, message: str):
        ...
        try:
            ...
            if subscriptions := self.plugin.parse_ws_subscription(data):
                # Add subscriptions

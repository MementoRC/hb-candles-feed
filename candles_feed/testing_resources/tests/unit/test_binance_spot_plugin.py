"""
Unit tests for the Binance Spot plugin.
"""
...
class TestBinanceSpotPlugin(unittest.TestCase):
    ...
    def test_format_ws_candle_message(self):
        ...
        result = self.plugin.format_ws_candle_message(
            self.candle, 'BTCUSDT', '1m', is_final=True
        )
    ...
    async def test_handle_exchange_info(self, mock_time):
        ...
        mock_server._simulate_network_conditions = MagicMock()
        mock_server._check_rate_limit = MagicMock(return_value=True)
        mock_server.trading_pairs = {
            'BTCUSDT': 50000.0,
            'ETHUSDT': 3000.0
        }
        ...
        # Act
        response = await self.plugin.handle_exchange_info(mock_server, mock_request)

...
class MockExchangeServer:
    ...
    def set_network_conditions(self, latency_ms: Optional[int] = None, 
    ...
    def set_rate_limits(self, rest_limit: Optional[int] = None, 
    ...
    async def start(self):
        ...
        self.logger.info(f"Mock {self.exchange_type.value} server started at http://{self.host}:{self.port}")
    ...
    def _check_rate_limit(self, ip: str, api_type: str) -> bool:
        ...
        if api_type == "rest":
            ...
            while (self.request_counts[api_type][ip] and 
                   now - self.request_counts[api_type][ip][0] > period_ms):
            ...
            while (self.request_counts[api_type][ip] and 
                   now - self.request_counts[api_type][ip][0] > 1000):
                self.request_counts[api_type][ip].popleft()
        ...
        try:
            ...
            if subscriptions:
                ...
                for trading_pair, interval in subscriptions:
                    ...
                    if trading_pair not in self.candles or interval not in self.candles[trading_pair]:

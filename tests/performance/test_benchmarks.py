...
class TestNetworkClientPerformance:
    ...
    async def test_concurrent_request_performance(self):
        ...
        try:
            ...
            assert result.avg_latency_ms < 100, f"Average latency too high: {result.avg_latency_ms:.2f}ms"
            assert result.p95_latency_ms < 200, f"P95 latency too high: {result.p95_latency_ms:.2f}ms"
...
class TestDataProcessorPerformance:
    ...
    def test_sanitize_candles_performance(self):
        ...
        # Performance targets
        assert metrics["avg_duration_ms"] < 50, f"Sanitization too slow: {metrics['avg_duration_ms']:.2f}ms"
    ...
    def test_process_candle_performance(self):
        ...
        assert metrics["avg_duration_ms"] < 5, f"Candle processing too slow: {metrics['avg_duration_ms']:.2f}ms"
...
class TestIntegrationPerformance:
    ...
    async def test_multiple_adapters_concurrent(self):
        ...
        async def concurrent_fetch():
            ...
            tasks = [
                adapter.fetch_rest_candles("BTCUSDT", "1m", limit=50)
        ...
        assert result.avg_latency_ms < 100, f"Concurrent performance too slow: {result.avg_latency_ms:.2f}ms"
        assert result.success_rate > 0.95, f"Success rate too low under load: {result.success_rate:.1%}"
        assert result.memory_peak_mb < 300, f"Memory usage too high under load: {result.memory_peak_mb:.1f}MB"
    ...
    async def test_sustained_load_performance(self):
        ...
        assert total_growth < 100, f"Memory leak detected: {total_growth:.1f}MB growth over 50 operations"

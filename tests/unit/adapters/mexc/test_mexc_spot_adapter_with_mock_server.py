"""
Unit tests for the MEXCSpotAdapter class using the mock exchange server.
"""
...
@pytest.fixture
async def mexc_mock_server():
    """
    Create and start a mock MEXC server, yield it, then stop it after the test.
    """
    # Create a mock server with specific trading pairs and initial prices
    server = create_mock_server_for_exchange(
        ExchangeType.MEXC_SPOT, "127.0.0.1", 0,
        [
            ("BTC-USDT", "1m", 50000.0),
            ("ETH-USDT", "1m", 3000.0),
            ("SOL-USDT", "1m", 100.0),
...
@pytest.fixture
def mock_server_url(mexc_mock_server):
    """
    Get the URL to the mock server.
    """
    return f"http://{mexc_mock_server.host}:{mexc_mock_server.port}"


class TestMEXCSpotAdapterWithMockServer:
    """Test suite for the MEXCSpotAdapter with mock server."""
    ...
    @pytest.mark.asyncio
    async def test_fetch_recent_candles(self, mexc_mock_server, mock_server_url):
    ...
    @pytest.mark.asyncio
    async def test_websocket_connection(self, mexc_mock_server, mock_server_url):
        ...
        async with aiohttp.ClientSession() as session, session.ws_connect(ws_url) as ws:
            ...
            assert response.get("channel") == "sub.response"
            assert response.get("data", {}).get("success") is True

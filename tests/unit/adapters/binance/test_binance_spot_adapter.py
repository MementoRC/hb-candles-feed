...
class TestBinanceSpotAdapter(BaseAdapterTest):
    ...
    def get_expected_ws_subscription_payload(self, trading_pair, interval):
        ...
        return {
            "method": "SUBSCRIBE",
            "params": [f"{self.get_expected_trading_pair_format(trading_pair).lower()}@kline_{INTERVAL_TO_EXCHANGE_FORMAT.get(interval, interval)}"],
            "id": 1,
        }
    ...
    # Additional test cases specific to BinanceSpotAdapter
    ...
    def test_binance_specific_trading_pair_format(self, adapter):
        """Test Binance-specific trading pair format."""
        ...
        assert adapter.get_trading_pair_format("SOL-USDT") == "SOLUSDT"

"""
Unit tests for the GateIoSpotAdapter class using the mock exchange server.
"""
...
@pytest.fixture
async def gate_io_mock_server():
    """
    Create and start a mock Gate.io server, yield it, then stop it after the test.
    """
    # Create a mock server with specific trading pairs and initial prices
    server = create_mock_server_for_exchange(
        ExchangeType.GATE_IO_SPOT, "127.0.0.1", 0,
        [
            ("BTC-USDT", "1m", 50000.0),
            ("ETH-USDT", "1m", 3000.0),
            ("SOL-USDT", "1m", 100.0),
...
@pytest.fixture
def mock_server_url(gate_io_mock_server):
    """
    Get the URL to the mock server.
    """
    return f"http://{gate_io_mock_server.host}:{gate_io_mock_server.port}"


class TestGateIoSpotAdapterWithMockServer:
    """Test suite for the GateIoSpotAdapter with mock server."""
    ...
    @pytest.mark.asyncio
    async def test_fetch_recent_candles(self, gate_io_mock_server, mock_server_url):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Create URL for Gate.io API
            url = f"{mock_server_url}/api/v4/spot/candlesticks"
    ...
    @pytest.mark.asyncio
    async def test_websocket_connection(self, gate_io_mock_server, mock_server_url):

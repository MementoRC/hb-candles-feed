            ...
            async with aiohttp.ClientSession() as session:
                for pair, interval, expected_price in trading_pairs:
                    ...
                    async with session.get(f"{url}/api/v3/klines", params=params) as response:
                        ...
                        if pair == "BTCUSDT":
                            assert 10000.0 <= latest_price <= 100000.0, f"BTC price {latest_price} out of reasonable range"
                        elif pair == "ETHUSDT":
                            assert 1000.0 <= latest_price <= 10000.0, f"ETH price {latest_price} out of reasonable range"
                        elif pair == "SOLUSDT":
                            assert 10.0 <= latest_price <= 1000.0, f"SOL price {latest_price} out of reasonable range"
                        elif pair == "DOGEUSDT":
                            assert 0.01 <= latest_price <= 10.0, f"DOGE price {latest_price} out of reasonable range"
                        else:

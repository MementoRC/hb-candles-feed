...
class TestAsyncThrottlerIntegration:
    ...
    def rate_limited_throttler(self):
        ...
        rate_limits = [
            ...
            {"limit_id": "heavy_operations", "limit": 2, "time_interval": 2.0},  # 2 calls per 2 seconds
        ]
    ...
    def configured_factory(self):
        ...
        rest_responses = {
            "https://api.binance.com/api/v3/klines": [
                [1625097600000, "35000.1", "35100.5", "34900.2", "35050.3", "10.5", 
                 1625097899999, "367000.5", 100, "5.2", "182000.1", "0"]
            ],
            "https://api.coinbase.com/v2/exchange-rates": {"data": {"currency": "BTC", "rates": {"USD": "35000"}}},
        }
        
        ws_messages = [
            json.dumps({
                "e": "kline", "E": 1625098200000, "s": "BTCUSDT",
                "k": {"t": 1625098200000, "o": "35150.2", "c": "35200.1", "h": "35250.0", "l": "35100.0"}
            })
        ]
        
        return MockWebAssistantsFactory(
...
class TestHummingbotNetworkClientIntegration:
    ...
    def integrated_components(self):
        ...
        return create_mock_hummingbot_components(
            rest_responses={
                "https://api.test.com/candles": [["1625097600000", "35000", "35100", "34900", "35050", "10.5"]],
            ...
            rate_limits=[
                {"limit_id": "api_requests", "limit": 10, "time_interval": 1.0}
            ]
        )
    ...
    async def test_network_client_rest_operations(self, integrated_components):
        ...
        with patch("candles_feed.core.hummingbot_network_client_adapter.HUMMINGBOT_AVAILABLE", True):
            ...
            response = await client.get_rest_data(
                url="https://api.test.com/candles",
                params={"symbol": "BTCUSDT", "interval": "1m"}
            )
    ...
    async def test_network_client_error_handling(self, integrated_components):
        ...
        with patch("candles_feed.core.hummingbot_network_client_adapter.HUMMINGBOT_AVAILABLE", True):
            ...
            with pytest.raises(Exception):  # Should raise an error for unknown URL
...
class TestNetworkClientFactory:
    ...
    async def test_factory_fallback_to_standalone(self):
        ...
        with patch("candles_feed.core.hummingbot_network_client_adapter.HUMMINGBOT_AVAILABLE", False):
...
class TestWSAssistantAdapter:
    ...
    async def test_ws_adapter_basic_operations(self, mock_ws_assistant):
        ...
        mock_ws_assistant.send.assert_called_once_with('{"method": "subscribe", "channel": "ticker"}')

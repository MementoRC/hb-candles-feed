...
class TestCandlesFeedIntegration:
    ...
    async def test_rest_strategy_integration(self, standalone_mock_server):
        ...
        try:
            ...
            assert all(timestamps[i] < timestamps[i+1] for i in range(len(timestamps)-1)), "Timestamps should be in order"
    ...
    async def test_multiple_feeds_integration(self, standalone_mock_server):
        ...
        try:
            ...
            assert btc_candles[-1].close > 0, f"BTC price {btc_candles[-1].close} should be positive"
            assert eth_candles[-1].close > 0, f"ETH price {eth_candles[-1].close} should be positive"
            assert sol_candles[-1].close > 0, f"SOL price {sol_candles[-1].close} should be positive"
            
            # Verify all feeds have different prices (they're different trading pairs)
            assert btc_candles[-1].close != eth_candles[-1].close, "BTC and ETH should have different prices"
            assert btc_candles[-1].close != sol_candles[-1].close, "BTC and SOL should have different prices"
            assert eth_candles[-1].close != sol_candles[-1].close, "ETH and SOL should have different prices"
    ...
    async def test_different_intervals_integration(self, standalone_mock_server):
        ...
        try:
            ...
            for interval, feed in interval_feeds:
                ...
                assert all(c.timestamp > 0 for c in candles), f"Timestamps should be positive for {interval}"
                assert all(c.open > 0 for c in candles), f"Open prices should be positive for {interval}"
                assert all(c.high > 0 for c in candles), f"High prices should be positive for {interval}"
                assert all(c.low > 0 for c in candles), f"Low prices should be positive for {interval}"
                assert all(c.close > 0 for c in candles), f"Close prices should be positive for {interval}"
                assert all(c.volume >= 0 for c in candles), f"Volume should be non-negative for {interval}"
    ...
    async def test_error_handling_integration(self, standalone_mock_server):
        ...
        try:
            ...
            logger.info(f"Initial candle count: {pre_ws_candle_count}, Final count: {len(final_candles)}")
    ...
    async def test_historical_data_fetch(self, standalone_mock_server):
        ...
        try:
            ...
            for candle in range_candles:
                assert start_time <= candle.timestamp <= end_time, "Candle outside requested time range"

...
def validate_candle_data_to_array():
    ...
    for input_ts, expected_ts in timestamp_tests:
        candle = CandleData(
            timestamp_raw=input_ts,
            open=100.0, high=101.0, low=99.0, close=100.5, volume=1000.0
        )
        array_result = candle.to_array()
        assert array_result[0] == expected_ts, f"Timestamp conversion failed for {input_ts}: {array_result[0]} != {expected_ts}"
...
def validate_dataframe_structure():
    ...
    with patch(
        ...
        assert list(df.columns) == expected_columns, f"Column mismatch: {list(df.columns)} != {expected_columns}"
        ...
        for col, allowed_types in expected_types.items():
            ...
            assert actual_type in allowed_types, f"Type mismatch for {col}: {actual_type} not in {allowed_types}"
...
def validate_timestamp_rounding():
    ...
    for input_ts, interval_sec, expected in test_cases:
        ...
        assert result_util == expected, f"Utility function failed: round_timestamp_to_interval({input_ts}, {interval_sec}) = {result_util}, expected {expected}"
        ...
        with patch(
            ...
            assert result_feed == expected, f"CandlesFeed method failed: _round_timestamp_to_interval_multiple({input_ts}) = {result_feed}, expected {expected}"
...
def validate_check_candles_sorted_and_equidistant():
    ...
    with patch(
        ...
        assert feed.check_candles_sorted_and_equidistant(), "Sorted equidistant candles should return True"
        ...
        assert feed.check_candles_sorted_and_equidistant(external_data), "External sorted data should return True"
        
        # Test 5: Unsorted data (should return False)
        unsorted_data = [
            [1609459260, 50100.0, 51100.0, 49100.0, 50600.0, 100.0, 0.0, 0, 0.0, 0.0],  # Later timestamp first
            [1609459200, 50000.0, 51000.0, 49000.0, 50500.0, 100.0, 0.0, 0, 0.0, 0.0],  # Earlier timestamp second
        ]
        
        assert not feed.check_candles_sorted_and_equidistant(unsorted_data), "Unsorted data should return False"
        
        # Test 6: Non-equidistant data (should return False)
        non_equidistant_data = [
            [1609459200, 50000.0, 51000.0, 49000.0, 50500.0, 100.0, 0.0, 0, 0.0, 0.0],
            [1609459230, 50100.0, 51100.0, 49100.0, 50600.0, 100.0, 0.0, 0, 0.0, 0.0],  # 30-second gap instead of 60
            [1609459320, 50200.0, 51200.0, 49200.0, 50700.0, 100.0, 0.0, 0, 0.0, 0.0],  # 90-second gap
        ]
        
        assert not feed.check_candles_sorted_and_equidistant(non_equidistant_data), "Non-equidistant data should return False"
...
def validate_backward_compatibility():
    ...
    original_data = [1609459200.0, 50000.0, 51000.0, 49000.0, 50500.0, 100.0, 5000000.0, 1000.0, 60.0, 3000000.0]
    ...
    for i, (original, converted) in enumerate(zip(original_data, converted_back)):
        if i == 7:  # n_trades gets converted to int then back to float
            assert int(original) == int(converted), f"n_trades conversion mismatch at index {i}: {original} != {converted}"
        else:
            assert original == converted, f"Roundtrip conversion mismatch at index {i}: {original} != {converted}"
    
    # Test dictionary conversion with various key formats
    dict_tests = [
        # Standard format
        {
            "timestamp": 1609459200, "open": 50000.0, "high": 51000.0, "low": 49000.0,
            "close": 50500.0, "volume": 100.0, "quote_asset_volume": 5000000.0,
            "n_trades": 1000, "taker_buy_base_volume": 60.0, "taker_buy_quote_volume": 3000000.0
        },
        # Short format (common in APIs)
        {"t": 1609459200, "o": 50000.0, "h": 51000.0, "l": 49000.0, "c": 50500.0, "v": 100.0},
        # Mixed format
        {"time": 1609459200, "open": 50000.0, "h": 51000.0, "l": 49000.0, "close": 50500.0, "volume": 100.0},
    ]
    ...
    for ts_format in timestamp_formats:
        ...
        assert candle.timestamp == 1609459200, f"Timestamp format {type(ts_format)} failed: {candle.timestamp}"

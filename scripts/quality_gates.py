...
class QualityGatesOrchestrator:
    ...
    def _configure_quality_checks(self) -> List[Dict[str, Any]]:
        ...
        if self.mode == "quick":
            return [
                check for check in base_checks if check.get("quick", False) and check["command"]
            ]
        elif self.mode == "ci":
    ...
    def _has_security_scan(self) -> bool:
        ...
        try:
            result = subprocess.run(
                ["pixi", "task", "list"], capture_output=True, text=True, timeout=10
            )
    ...
    def run_quality_gates(self) -> QualityGateReport:
        ...
        for check_config in self.quality_checks:
            result = self._run_command(check_config["command"], check_config["name"])
    ...
    def _save_json_report(
        self, report: QualityGateReport, filename: str = "quality_gates_report.json"
    ):
...
def main():
    ...
    try:
        orchestrator = QualityGatesOrchestrator(mode=args.mode, output_format=args.output)
